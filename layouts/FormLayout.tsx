...
export default function Form({
  ...
  const mergedInitial = { ...defaultState, ...initialValues, ...reportData };
  const [formData, setFormData] = useState(mergedInitial);

  useEffect(() => {
    setFormData({ ...defaultState, ...initialValues, ...reportData });
  ...
  const handleFocus = (field: Field) => {
    if (field.noKeyboard) {
      ...
      if (field.getTargetName) field.getTargetName(field.name);
    }
  };

  const commonInputClasses =
    'peer w-full border border-gray-300 rounded-md p-4 text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-400';
